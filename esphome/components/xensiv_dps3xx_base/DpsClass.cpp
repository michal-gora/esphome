...
void DpsClass::begin(TwoWire &bus) { begin(bus, DPS__STD_SLAVE_ADDRESS); }
...
void DpsClass::begin(TwoWire &bus, uint8_t slaveAddress) {
  ...
  delay_ms(50);  // startup time of Dps3xx
...
int16_t DpsClass::measureTempOnce(float &result, uint8_t oversamplingRate) {
  ...
  delay_ms(calcBusyTime(0U, m_tempOsr) / DPS__BUSYTIME_SCALING);
  delay_ms(DPS3xx__BUSYTIME_FAILSAFE);
...
int16_t DpsClass::measurePressureOnce(float &result, uint8_t oversamplingRate) {
  ...
  // wait until measurement is ready
  delay_ms(calcBusyTime(0U, m_prsOsr) / DPS__BUSYTIME_SCALING);
  delay_ms(DPS3xx__BUSYTIME_FAILSAFE);
  ret = getSingleResult(result);
  if (ret != DPS__SUCCEEDED) {
    standby();
